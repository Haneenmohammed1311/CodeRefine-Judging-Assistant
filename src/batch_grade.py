"""
Grades every team in a CSV file, instead of typing one --team/--repo pair
at a time. This is the realistic way many teams actually get graded: they
fill out a submission form (e.g. a Google Form), you export the responses
...
Expected CSV format (a header row, then one row per team):
    ...
    Team Beta,https://github.com/beta-team/coderefine
        ...
        while True:
            ...
            try:
                ...
                if _is_rate_limit_error(e) and attempt <= MAX_RETRIES_ON_RATE_LIMIT:
                    ...
                    time.sleep(wait)
                    continue
                # Either a genuine failure (bad URL, private repo, etc.),
