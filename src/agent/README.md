...
`llm.py`: creates the Groq client the agent uses to think. One function, `get_llm()`, cached by temperature and by whether JSON mode is on, so repeated calls don't reconnect every time. Grading and feedback use `temperature=0.0` (consistent, repeatable results); the chatbot uses a slightly higher one elsewhere. `json_mode=True` turns on Groq's JSON mode, which guarantees the response is syntactically valid JSON instead of hoping the model follows a plain text instruction to output JSON.
...
- `gather_node`, reads the repo's README, Deep Dives file, BOTE file, and architecture diagram, and produces raw observations (no scoring). Includes prompt injection defenses, since this content is written by the team being graded and shouldn't be trusted as instructions. Shared by both the practice and official paths.
